...
- アーカイブ展開による一括配置
- 巨大ファイルのチャンク再開
- MCP関連機能（認証フローを含む）の設計・実装（別プロダクトで扱う）
...
- ファイル名が `vp.php` 固定のままだと第三者に認証入口を推測される恐れがあるため、ユーザー向けドキュメントでは `vp.php` の `vp` 部分を任意名へリネームする運用を推奨する
...
- ログイン/セットアップ入口は `login_token` 付きURLに限定する
  - トークンは初回起動時に128bit以上の乱数で生成し、別ファイルで保持する
  - `?login_token=...` が一致する場合のみ、ログイン/セットアップ画面を表示する
  - トークンなし・不一致アクセスは常に同一の404応答を返す
  - トークン保存ファイルが削除されている場合は、次回起動時に新トークンを再生成する
  - セットアップ完了後は通常画面でトークンを再表示しない
...
- ログイン失敗回数の加算は、`login_token` 一致後の試行のみを対象とする
...
7. 失敗のみ再送が機能する
8. `login_token` なし/不一致アクセスでは常に404で、ログイン画面が表示されない
9. `login_token` 一致時のみログイン/セットアップ画面に到達できる
10. ログイン失敗回数は `login_token` 一致後の試行でのみ増加する
11. `vp.php` が任意ファイル名へリネームされていても、ログイン・同期・dry-run が完全に動作するテストで確認できる
